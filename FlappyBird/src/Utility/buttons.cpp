...
namespace flappybird {
	namespace buttons {
		...
		void draw(BTN button, const char *text) {
			...
			DrawText(text, button.size.x - (MeasureText(text, button.fontsize) / 2) 
						+ button.size.width/2, button.size.y  - (button.fontsize / 2) 
						+ button.size.height/2, button.fontsize, button.textColor);
